...
export const AppDataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const refreshAlerts = async () => {
    try {
      const endpoint = '/alert/my-alerts';
  ...
  const addCrop = async (cropData: any) => {
    try {
      ...
      await refreshCrops();
      showToast('Crop added to database', 'success');
    } catch (err) {
  ...
  const setActiveCrop = async (cropId: string) => {
    try {
      ...
      await refreshUser();
      await refreshCrops();
      showToast('Active crop updated successfully', 'success');
    } catch (err) {
      console.error('setActiveCrop error:', err);
      showToast('Failed to update active crop', 'error');
    }
  };
